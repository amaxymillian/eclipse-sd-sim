...
var SAVED_SHIPS_KEY = 'eclipse-sd-sim-ships';

function getSavedShips() {
    try {
        var data = localStorage.getItem(SAVED_SHIPS_KEY);
...
function persistSavedShips(ships) {
    try {
        localStorage.setItem(SAVED_SHIPS_KEY, JSON.stringify(ships));
    } catch (e) {
...
function serializeCurrentSlots() {
    var slots = blueprintSlotBoxes[currentBlueprintName];
    if (!slots) return [];
    var result = [];
    for (var i = 0; i < slots.length; i++) {
...
function saveCurrentShip() {
    ...
    ships.push({
        name: name,
        blueprint: currentBlueprintName,
...
function loadSavedShip(index) {
    ...
    var waitInterval = setInterval(function() {
        waitCount++;
        var slots = blueprintSlotBoxes[ship.blueprint];
...
function renameSavedShip(index) {
    var ships = getSavedShips();
...
function deleteSavedShip(index) {
    var ships = getSavedShips();
    if (index < 0 || index >= ships.length) return;
...
function importShipFromEvent(event) {
